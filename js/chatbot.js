...
(function (window, document) {
  ...
  function detectIntent(raw) {
    ...
    if (/\breview\b|what do (people|customers|others) (say|think)|customer feedback|rated well/i.test(text)) {
    ...
    if (/room preview|(see|show me) (this |it )?in my room|how (would|will) (this|it) look in my room|preview in my space|visualize (this |it )?in my (room|home|space)/i.test(text)) {
      return { type: "room_preview" };
    }
  ...
  class ShopSphereChat {
    ...
    _handleRoomPreview() {
      ...
      const product = this.lastResults[0];
      this._pushBotMessage(`Opening Room Preview for "${product.name}" — this is a simple photo mockup, not real AR, but it'll give you a rough sense of scale and placement.`);
    ...
    _openRoomPreviewModal(product) {
      ...
      modal.innerHTML = `
        <div class="ssc-room-panel">
          ...
          <div class="ssc-room-hint">Drag the item to position it. This is a rough visual aid only — actual size, lighting, and color may differ.</div>
        </div>
      `;
